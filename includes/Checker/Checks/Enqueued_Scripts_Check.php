<?php
/**
 * Class WordPress\Plugin_Check\Checker\Checks\Enqueued_Scripts_Check
 *
...
/**
 * Check for controlling the amount of enqueued scripts.
 *
 * @since 1.0.0
 */
class Enqueued_Scripts_Check extends Abstract_Request_Check {
	...
	public function prepare() {
		$cleanup = parent::prepare();

		$orig_scripts = isset( $GLOBALS['wp_scripts'] ) ? $GLOBALS['wp_scripts'] : null;

		return function() use ( $cleanup, $orig_scripts ) {
			$cleanup();
			if ( $orig_scripts ) {
	 ...
	 *                   the check).
	 */
	protected function run_for_url( Check_Result $result, $url ) {
